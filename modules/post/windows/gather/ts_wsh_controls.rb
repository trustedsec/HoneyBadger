...
class MetasploitModule < Msf::Post
  ...
  def run()
    print_status("Running post module ts_wsh_controls against #{sysinfo['Computer']} in session #{datastore['SESSION']}")
    ...
    if settings.include?('UseWINSAFER')
      ...
      if policy_setting == "1"
        ...
        report_note(
            ...
            :type   => 'host.control.wsh',
            :data   => {
                :control => "policy_source",
                :source => "SRP"},
            :update => :unique_data
        )
      else
